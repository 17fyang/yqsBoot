...
public class FatherGoodService {
    ...
    public JSONObject newTransaction(MultipartFile[] file, String name, String describe, String tag, Double price, Double originalPrice, Double postage, String freeShipping, int goodType) throws LogicException {
        ...
        if (file == null || file.length == 0) throw new LogicException(501, "图片参数不得为空");
...
//		file[1]=temp;
        ...
        for (MultipartFile multipartFile : file) {
            imageUtil.checkFileSize(multipartFile);
            String[] newUrl = imageUtil.newFileUrl("goodImage", multipartFile);
            String localUrl = newUrl[0];
            String httpUrl = newUrl[1];
            imageUtil.compressFile(multipartFile, localUrl);
            urlList.add(httpUrl);
        }
        String[] url = urlList.toArray(new String[urlList.size()]);
